...
public class TableroRepository : ITableroRepository
{
    ...
    public bool Update(Tablero tableroModificar){
        bool tableroModificado = false;
        using (SQLiteConnection connection = new SQLiteConnection(cadenaConexion))
        {
            connection.Open();
            ...
            if(command.ExecuteNonQuery() > 0) tableroModificado=true;
            connection.Close();
        }
    ...
    public bool Remove(int idTablero){
        ...
        using (SQLiteConnection connection = new SQLiteConnection(cadenaConexion))
        {
            ...
            SQLiteCommand command = new SQLiteCommand(query, connection);
            command.Parameters.Add(new SQLiteParameter("@idTab", idTablero));
            if (command.ExecuteNonQuery() > 0) tableroEliminado = true;
            connection.Close();
        }
